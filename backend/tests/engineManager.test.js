...
function makeSpawnFn() {
    ...
    const spawnFn = mock((opts) => {
        ...
        const proc = {
            ...
            kill: mock(),
            _pushLine: (line) => {
                const oldResolve = resolveNext;
                nextPromise = new Promise(r => resolveNext = r);
                oldResolve(line);
            },
            _close: () => resolveNext(null)
        };
        processes.push(proc);
        return proc;
    });
    spawnFn.processes = processes;
    return spawnFn;
}
...
describe("UciEngine.start()/.stop()",  () => {
    ...
    it.only("sends 'setoption name OwnBook value true' and 'setoption name BookFile value <bookPath>' if bookPath is configured", async () => {
...
//     });
//
//     it.only("handshake timeout triggers a restart attempt", async () => {
//         const spawnFn = makeSpawnFn();
