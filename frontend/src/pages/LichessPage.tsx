                            ...
                            <div key={t.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-950 border border-slate-800">
                                <div className="min-w-0">
                                    <div className="text-sm text-slate-200 truncate flex items-center gap-2">
                                        <a href={`https://lichess.org/${t.type === "swiss" ? "swiss" : "tournament"}/${t.id}`} target="_blank" rel="noreferrer" className="hover:underline truncate">{t.name}</a>
                                        <span className="text-[10px] uppercase tracking-wide text-slate-500 border border-slate-700 rounded px-1 shrink-0">{t.type}</span>
                                    </div>
                                    <div className="text-xs text-slate-500 truncate">
                                        {tournamentWhen(t)} · {t.nbPlayers ?? 0} players
                                        {t.teamId && <> · team <a href={`https://lichess.org/team/${t.teamId}`} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline">{t.teamName || t.teamId}</a></>}
                                    </div>
                                </div>
